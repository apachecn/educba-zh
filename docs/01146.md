...
**说明:**在上面的程序中，定义了一个名为 dat1 的变量来存储一个日期。然后定义一个名为 dat2 的变量来存储另一个日期。然后使用 boolean after(Date date)方法检查 dat1 是否在 dat2 日期之后，返回的输出是真还是假。然后使用 boolean before(Date date)方法检查 dat1 是否在 dat2 日期之前，返回的输出是 true 还是 false。
