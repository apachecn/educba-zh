...
这是在 2013 年作为 OpenGL 的稳定版本发布的，其中包含一些新功能，即在缓冲区处于映射形式时使用缓冲区的能力，它可以作为缓冲区对象的不可变存储。在这个版本中，纹理图像可以直接清除，并且还为输入/输出变量提供紧密封装。不仅是紧密封装，还有一个选项可以设置输入/输出接口块上的位置，以便进行封装。在一次调用中，可以完成索引绑定的相同类型的顺序范围内的对象数组的绑定。通过特殊的夹紧模式特征，每个维度上的纹理尺寸可以加倍，并且镜像也可以在纹理的精确负坐标方向上找到。对于纹理，它还提供仅模板图像格式，并且需要 8 位模板格式。对于顶点属性，它提供了一个打包的三分量格式，即 11F/11F/10F。
