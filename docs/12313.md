...
soup 对象使用 HTML 搜索和导航您想要的数据。find 和 findall 是最强大的模块之一。在只有一个元素的地方，您知道 body 标签有 soup.find()。而 soup.findall()则用于网络抓取的冒险。使用它，迭代和打印所有超链接的 URL 就完成了。此外，标记属性和使用 findall 提供不同的参数就像正则表达式一样，可以根据您的需要进行具体更改。
