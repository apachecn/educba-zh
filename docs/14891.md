...
根据特定项目的要求，JDBC 使用简单的数据库查询或简单的查询语言与特定的数据库进行通信。所以对于多个关系数据库来说，它永远不会是独立的。在这种情况下，开发人员还可以开发一种特定的设计，在 JDBC 的情况下，通过在多个属性文件中维护各个数据库的各种查询，并在加载特定应用程序时映射它们，为多个数据库使用相同的代码库。多个属性文件将保存特定关系数据库 SQL 查询的需求。但同样，这对开发者来说是一笔很大的成本，而且不容易做到。此外，每次在转移到另一个关系数据库之前，属性文件都需要更新。
