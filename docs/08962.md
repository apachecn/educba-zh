...
以下文章提供了 Java RandomAcessFile 的概要。java RandomAccessFile 是一个用于读写随机访问文件的类。RandomAccessFile 类是 java 中的一个内置类，它定义了 java.io.RandomAccessFile 包。随机访问文件类似于存储在文件系统实例中的字节数组。这个文件维护一个光标或指针；通过将指针移动到特定位置，我们可以对该特定位置执行读或写操作。当文件在读取所需的字节数之前到达文件末尾时，将引发 EOF 异常。
