...
**答案:**
线程是进程的子集。一个进程可以运行多个线程。线程可以运行进程的任何部分。一个进程有自己的存储地址，而线程共享创建它的进程的地址。创建进程是一个繁忙的过程，而创建线程却很容易。线程通常被称为轻量级进程。进程间通信很困难，而线程可以使用 wait()和 notify()的 Java 方法轻松地进行通信。过程是独立的；因此，对流程所做的任何更改都不会影响子流程。相反，如果在一个线程中进行了更改，其他线程也有可能受到影响。
